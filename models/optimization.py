...
def create_optimizer(
    ...
    if weight_decay_rate > 0.0:
        ...
        if optimizer_type =="adam":
            ...
            optimizer=tf.keras.optimizers.SGD(learning_rate=lr_schedule,momentum=adam_beta1)
    # We return the optimizer and the LR scheduler in order to better track the
    # evolution of the LR independently of the optimizer.
    return optimizer, lr_schedule


class SGDWeightDecay(tf.keras.optimizers.SGD):
    def __init__(self,
        ...
        **kwargs):
        super(SGDWeightDecay, self).__init__(learning_rate,beta_1,name, **kwargs)
    ...
    def from_config(cls, config):
        ...
        return super(SGDWeightDecay, cls).from_config(config, custom_objects=custom_objects)
      
    def _prepare_local(self, var_device, var_dtype, apply_state):
        super(SGDWeightDecay, self)._prepare_local(var_device, var_dtype, apply_state)
    ...
    def apply_gradients(self, grads_and_vars, name=None, **kwargs):
        ...
        return super(SGDWeightDecay, self).apply_gradients(zip(grads, tvars), name=name, **kwargs)
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        with tf.control_dependencies([decay]):
            return super(SGDWeightDecay, self)._resource_apply_dense(grad, var, **kwargs)
    def _resource_apply_sparse(self, grad, var, indices, apply_state=None):
        lr_t, kwargs = self._get_lr(var.device, var.dtype.base_dtype, apply_state)
        decay = self._decay_weights_op(var, lr_t, apply_state)
        with tf.control_dependencies([decay]):
            return super(SGDWeightDecay, self)._resource_apply_sparse(grad, var, indices, **kwargs)
